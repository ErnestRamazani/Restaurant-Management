  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-5 pt-[max(0.5rem,env(safe-area-inset-top))]">
        ...
        <div className="mt-3 w-full max-w-[280px] rounded-2xl border border-gold/35 bg-[var(--gold-dim)] px-4 py-3 text-center shadow-[0_12px_40px_rgba(0,0,0,0.2)]">
          <p className="font-body text-[0.58rem] font-semibold uppercase tracking-[0.2em] text-gold/85">
            {t('guest.reservation.confirm.confirmationCode')}
          </p>
          {code ? (
            ...
            <p className="mt-2 font-body text-xs leading-relaxed text-champagne/55">
              {t('guest.reservation.confirm.codeUnavailable')}
            </p>
          )}
        </div>
        ...
        {ticket ? (
          <div className="mt-3 w-full max-w-[280px] rounded-xl border border-champagne/12 bg-midnight-2/90 px-3 py-2.5 font-body text-[0.72rem] leading-snug text-champagne/75">
            <p>
              ...
              {partySize != null ? (
                <span> · {t('guest.reservation.confirm.guests', { count: partySize })}</span>
              ) : null}
            </p>
            {ticket.phone ? (
              <p className="mt-0.5 truncate text-champagne/60">
                {t('guest.reservation.confirm.tel')} {ticket.phone}
              </p>
            ) : null}
            {ticket.arrivalLabel ? (
              <p className="mt-0.5 text-champagne/60">
                {t('guest.reservation.confirm.arrival')} {ticket.arrivalLabel}
              </p>
            ) : null}
            {ticket.tableLabel ? (
              <p className="mt-0.5 font-semibold text-champagne">
                {t('guest.reservation.confirm.table')} {ticket.tableLabel}
              </p>
            ) : null}
            {ticket.userNotes ? (
              <p className="mt-0.5 line-clamp-2 text-champagne/55">
                {t('guest.reservation.confirm.notes')} {ticket.userNotes}
              </p>
            ) : null}
          </div>
        ) : null}
        ...
        {ticket ? (
          ...
          >
            ...
            {t('guest.reservation.confirm.downloadPdf')}
          </button>
        ) : null}
      </div>
      ...
      <div className="shrink-0 space-y-2 px-5 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2">
        ...
        >
          {t('guest.reservation.confirm.newReservation')}
        </button>
        {onBackToStart ? (
          <button
            type="button"
          ...
          >
            {t('guest.reservation.confirm.backHome')}
          </button>
        ) : null}
      </div>
    </motion.div>
