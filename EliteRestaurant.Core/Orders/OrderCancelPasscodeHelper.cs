...
public static class OrderCancelPasscodeHelper
{
    public static string ResolveConfigured(AppDbContext db, int? restaurantId = null)
    {
        ...
        var fromDb = query.Select(s => s.OrderCancelPasscode).FirstOrDefault();
        var configured = string.IsNullOrWhiteSpace(fromDb)
            ? SettingsManager.Load().BusinessProfile.OrderCancelPasscode
            : fromDb;
    ...
    public static string? Validate(AppDbContext db, string? submitted)
    {
        ...
        {
            return "Order cancel passcode is not configured. Set it in Elite Pro → Appearance → Business profile, then push to cloud.";
        }

        var pass = StaffLoginPasscodeHelper.NormalizeSubmitted(submitted);
        if (!string.Equals(pass, expected, StringComparison.Ordinal))
            return "Incorrect cancel passcode.";
